...
Component::Component(const std::string &_name, const objectTransform &_trans, const Vector3<GLfloat> &_anchorPoint, Component *_parent, const int _zIndex, const size_t _frameCount)
    : name(_name), parent(_parent), zIndex(_zIndex), frameCount(_frameCount)
{
    ...
    trans.SetRotate  (trans.Rotate.x + parent->trans.Rotate.x, trans.Rotate.y + parent->trans.Rotate.y, trans.Rotate.z + parent->trans.Rotate.z);
    trans.SetScale   (trans.Scale.x * parent->trans.Scale.x, trans.Scale.y * parent->trans.Scale.y, trans.Scale.z * parent->trans.Scale.z);

    // anchorPoint.SetTransform();
}
