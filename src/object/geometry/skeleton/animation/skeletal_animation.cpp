#include <object/geometry/skeleton/animation/skeletal_animation.hpp>
#include <tiny_gltf.h>


glm::vec3 BoneKeyframes::CalcInterpolatedPosition(float AnimationTime) const {
    if (Positions.empty())
...
glm::quat BoneKeyframes::CalcInterpolatedRotation(float AnimationTime) const {
    if (Rotations.empty())
...
glm::vec3 BoneKeyframes::CalcInterpolatedScaling(float AnimationTime) const {
    if (Scales.empty())
        return glm::vec3(0.0f);
...
SkeletalAnimation::SkeletalAnimation(const aiAnimation* anim, const std::unordered_map<std::string, int>& BoneMap) {
...
SkeletalAnimation::SkeletalAnimation(const tinygltf::Model& model, const tinygltf::Animation& anim,
    ...
    for (size_t i = 0; i < anim.channels.size(); ++i) {
        ...
        auto it = NodeToBoneIndex.find(nodeIndex);
        ...
        int count = inputAccessor.count;
        Duration = std::max((float)Duration, inputData[count - 1]);
        ...
        for (int j = 0; j < count; ++j) {
            ...
            if (path == "translation") { // НЕ СЧИТЫВАЕТ СМЕЩЕНИЕ
                ...
                glm::quat rot(outputData[j * 4 + 3], outputData[j * 4 + 0],
                              outputData[j * 4 + 1], outputData[j * 4 + 2]);
                keyframes.Rotations.emplace_back(time, rot);
            }
            else if (path == "scale") {
                glm::vec3 scale(outputData[j * 3 + 0], outputData[j * 3 + 1], outputData[j * 3 + 2]);
                keyframes.Scales.emplace_back(time, scale);
            }
        }

        int boneIndex = it->second;
        Tracks[boneIndex] = std::move(keyframes);
    }
}
