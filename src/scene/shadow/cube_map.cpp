...
constexpr float kFarPlane  = 128.0f;
...
ShadowCubeMap::ShadowCubeMap(GLsizei resolution)
...
void ShadowCubeMap::ShadowPass(const Scene* scene, const glm::vec3 l) {
    ...
    glm::mat4 shadowProj = PerspectiveLH(glm::radians(90.0f), 1.0f, nearPlane, farPlane);
    ...
    PrintMatrix(shadowProj);
    PrintMatrix(LookAtLH(glm::vec3( 0, 0, 0), glm::vec3( 0,-1, 0), glm::vec3(0, 0,-1)));
    PrintMatrix(shadowTransforms[3]);
    std::cout << std::endl;
    ...
    static bool f = true;
    if (f) {
        SaveDepthCubeMap(map, SHADOW_SIZE, "cube_map");
        f = false;
    }
