...
public:
    ...
    SkeletalMesh(const std::string& path, Shader* shader);

    const Skeleton& GetSkeleton() const;
    
    static SkeletalMesh* Create(const std::string& name, const SkeletalMeshData& meshData, Shader* shader);
    static SkeletalMesh* Create(const std::string& name, const std::string& path, Shader* shader);
...
protected:
    void InitAttributes(const SkeletalMeshData& meshData);
};
